...
static int ft_servermode = FALSE;
int ft_batchmode = FALSE;
//bool ft_pipemode = FALSE;
int ft_pipemode = FALSE;
//bool rflag = FALSE; /* has rawfile */
int rflag = FALSE; /* has rawfile */
...
int ft_intrpt = FALSE;     /* Set by the (void) signal handlers. TRUE = we've been interrupted. */
int ft_setflag = FALSE;    /* TRUE = Don't abort simulation after an interrupt. */
...
int orflag = FALSE; /* global for -o option */
#endif
...
static int read_initialisation_file(char *dir, char *name);
...
//    bool qflag = FALSE;
    ...
    volatile int readinit = TRUE;
    volatile int istty = TRUE;
    int iflag = FALSE;
    int qflag = FALSE;
...
// initialization and seed for FastNorm Gaussian random generator
    ...
    printf("3cp_out:%x\n",(int)cp_out);
    printf("3cp_err:%x\n",(int)cp_err);
    cp_curin = (FILE *)argv[3];
    cp_curout = (FILE *)argv[4];
    cp_curerr = (FILE *)argv[5];
    cp_inp_cur = cp_in;
