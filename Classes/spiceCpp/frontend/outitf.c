...
#include "outToCpp.h"
extern void outToCppClassLineCount(int count);
...
static void fileStartPoint(FILE *fp, int bin, int num);
static void fileAddRealValue(FILE *fp, int bin, double value);
static void fileAddComplexValue(FILE *fp, int bin, IFcomplex value);
static void fileEndPoint(FILE *fp, int bin);
...
static int parseSpecial(char *name, char *dev, char *param, char *ind);
static int name_eq(char *n1, char *n2);
static int getSpecial(dataDesc *desc, runDesc *run, IFvalue *val);
...
static int shouldstop = FALSE; /* Tell simulator to stop next time it asks. */

//static bool interpolated = FALSE;
static int interpolated = FALSE;
...
static int
beginPlot(JOB *analysisPtr, CKTcircuit *circuitPtr, char *cktName, char *analName, char *refName, int refType, int numNames, char **dataNames, int dataType, int windowed, runDesc **runp)
{
    runDesc *run;
    ...
    if (dataType == 666 && numNames == 666) {
        ...
        *runp = run = alloc(struct runDesc);
        ...
        if (numsaves) {
            ...
            savesused = TMALLOC(int, numsaves);
            saveall = FALSE;
...
static void
fileStartPoint(FILE *fp, int bin, int num)
{
    if (!bin)
        fprintf(fp, "%d\t", num - 1);
...
static void
fileAddRealValue(FILE *fp, int bin, double value)
{
    if (bin)
        rowbuf[column++] = value;
    else
...
static void
fileAddComplexValue(FILE *fp, int bin, IFcomplex value)
{
    if (bin) {
...
fileEnd(runDesc *run)
{
    if (run->fp != stdout) {
        ...
        fprintf(stdout, "\nNo. of Data Rows : %d\n", run->pointCount);
        outToCppClassLineCount(run->pointCount);
