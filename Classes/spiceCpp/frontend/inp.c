...
static int doedit(char *filename);
...
void line_free_x(struct line *deck, int recurse);
void create_circbyline(char *line);

void inp_evaluate_temper(void);

//extern bool ft_batchmode;
extern int ft_batchmode;
...
com_listing(wordlist *wl)
{
    ...
    int expand = FALSE, do_param_listing = FALSE;
    char *s;
...
void
line_free_x(struct line *deck, int recurse)
{
    while (deck) {
...
void
inp_spsource(FILE *fp, int comfile, char *filename, int intfile)
/* arguments:
...
inp_dodeck(
    ...
    //bool reuse,            /*in: TRUE if called from runcoms2.c com_rset,FALSE if called from inp_spsource() */
    int reuse,            /*in: TRUE if called from runcoms2.c com_rset,FALSE if called from inp_spsource() */
    ...
    if (!noparse) {
        ...
        */
        for (eev = ct->ci_vars; eev; eev = eev->va_next) {
            ...
            int one = TRUE;   /* FIXME, actually eev->va_bool should be TRUE anyway */
            switch (eev->va_type) {
            case CP_BOOL:
