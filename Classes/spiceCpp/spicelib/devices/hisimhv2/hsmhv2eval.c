...
/*  HSMHV2hereMKSParam  *hereMKS = &here->hereMKS ;*/
  /*-----------------------------------*
...
/*  const double psia2_dlt = 5.0e-3 ;*/
  ...
  /* const double Vbs_min = model->HSMHV2_vbsmin ; */
  ...
  /* Substrate Effect */
  const double RDVSUB =  model->HSMHV2_rdvsub ; 
  const double RDVDSUB = model->HSMHV2_rdvdsub ;
  const double DDRIFT =  model->HSMHV2_ddrift ; 
  const double VBISUB =  model->HSMHV2_vbisub ;
  const double NSUBSUB = modelMKS->HSMHV2_nsubsub ;
  double Vsubsrev = 0.0 ;
...
#include "hsmhv2temp_eval.h"
  ...
  beta_inv = here->HSMHV2_beta_inv ;
  beta2 = here->HSMHV2_beta2 ;

  /* Bandgap */
  Egp12 = here->HSMHV2_egp12 ;
  Egp32 = here->HSMHV2_egp32 ;

  /* Metallurgical channel geometry */
  Leff = here->HSMHV2_leff ;
  Weff = here->HSMHV2_weff ;
  WeffLD_nf = here->HSMHV2_weff_ld * here->HSMHV2_nf ;

  Ldrift0 = here->HSMHV2_ldrift1 +  here->HSMHV2_ldrift2 ;

  /* Flat band voltage */
  Vfb = pParam->HSMHV2_vfbc ;

  /* Surface impurity profile */
  q_Nsub = here->HSMHV2_qnsub ;
  
  /* satulation Velocity */
  Vmax = here->HSMHV2_vmax ;
  
   
  /* 2 phi_B */
  Pb2 = here->HSMHV2_pb2 ;
  Pb20 = here->HSMHV2_pb20 ; 
  Pb2c = here->HSMHV2_pb2c ;

  /* Coefficient of the F function for bulk charge */
  cnst0 = here->HSMHV2_cnst0 ;

  /* cnst1: n_{p0} / p_{p0} */
  cnst1 = here->HSMHV2_cnst1 ;

  /* c_eox: Permitivity in ox  */
  c_eox = here->HSMHV2_cecox ;

  /* Tox and Cox without QME */
  Tox0 = model->HSMHV2_tox ;
  Cox0 = c_eox / Tox0 ;
  Cox0_inv = 1.0 / Cox0 ;

  /* for calculation of Ps0_min */
  Vgs_min = model->HSMHV2_type * model->HSMHV2_vgsmin ;

  /*---------------------------------------------------*
   * Determine clamping limits for too large Vbs (internal).
   *-----------------*/

  Fn_SU( T1 , Pb2  - model->HSMHV2_vzadd0 , Vbs_max , 0.1 , T0 ) ;
  Vbs_max = T1 ;
  Vbs_max_dT = Pb2_dT * T0 ;

  if ( Pb20 - model->HSMHV2_vzadd0 < Vbs_max ) {
    Vbs_max = Pb20 - model->HSMHV2_vzadd0 ;
    Vbs_max_dT = 0.0 ;
  }
  if ( Pb2c - model->HSMHV2_vzadd0 < Vbs_max ) {
    Vbs_max = Pb2c - model->HSMHV2_vzadd0 ;
    Vbs_max_dT = 0.0 ;
  }
  ...
  if (here->HSMHV2_rs > 0.0 || here->HSMHV2_rd > 0.0) {
    if ( model->HSMHV2_corsrd == 1 ) flg_rsrd  = 1 ;
    if ( model->HSMHV2_corsrd == 2 ) flg_rsrd  = 2 ;
    if ( model->HSMHV2_corsrd == 3 ) flg_rsrd  = 3 ;
  }
  ...
  if ( here->HSMHV2_called >= 1 ) {

    Vbsc_dif = Vbs - here->HSMHV2_vbsc_prv ;
    Vdsc_dif = Vds - here->HSMHV2_vdsc_prv ;
    Vgsc_dif = Vgs- here->HSMHV2_vgsc_prv ;

    sum_vdif  = fabs( Vbsc_dif ) + fabs( Vdsc_dif ) 
              + fabs( Vgsc_dif ) ;

    if ( model->HSMHV2_copprv >= 1 && sum_vdif <= vtol_pprv  &&
         here->HSMHV2_mode * here->HSMHV2_mode_prv > 0 ) { flg_pprv = 1 ;}

    if ( here->HSMHV2_called >= 2 && flg_pprv == 1 ) {
      Vbsc_dif2 = here->HSMHV2_vbsc_prv - here->HSMHV2_vbsc_prv2 ;
      Vdsc_dif2 = here->HSMHV2_vdsc_prv - here->HSMHV2_vdsc_prv2 ;
      Vgsc_dif2 = here->HSMHV2_vgsc_prv - here->HSMHV2_vgsc_prv2 ;
      sum_vdif2 = fabs( Vbsc_dif2 ) + fabs( Vdsc_dif2 ) 
                  + fabs( Vgsc_dif2 ) ;
      if ( epsm10 < sum_vdif2 && sum_vdif2 <= vtol_pprv &&
           here->HSMHV2_mode_prv * here->HSMHV2_mode_prv2 > 0 ) { flg_pprv = 2 ; }
    }
    Temp_dif = TTEMP - here->HSMHV2_temp_prv ;
  ...
  if ( flg_pprv >= 1 ) {
    Ps0 = here->HSMHV2_ps0_prv ;
    Ps0_dVbs = here->HSMHV2_ps0_dvbs_prv ;
    Ps0_dVds = here->HSMHV2_ps0_dvds_prv ;
    Ps0_dVgs = here->HSMHV2_ps0_dvgs_prv ;
  
    Pds = here->HSMHV2_pds_prv ;
    Pds_dVbs = here->HSMHV2_pds_dvbs_prv ;
    Pds_dVds = here->HSMHV2_pds_dvds_prv ;
    Pds_dVgs = here->HSMHV2_pds_dvgs_prv ;
  }

  if( here->HSMHV2_cordrift == 1 && here->HSMHV2subNode >= 0 ) {
    /*----------------------------------------------------------*
     * Considering these special cases:
     * ( here->HSMHV2_mode == HiSIM_NORMAL_MODE  && Vdse < 0.0 )
     * ( here->HSMHV2_mode == HiSIM_REVERSE_MODE && Vdse < 0.0 )
     *----------------------------------------------------------*/

    Vdsegmt = here->HSMHV2_mode * Vdse ; /* geometrical outer bias */
    ...
    here->HSMHV2_Vsubsrev = Vsubsrev ;

    /*-----------------------------------------------------------*
     * Vxserevz: Modified bias introduced to realize symmetry at Vds=0.
     *-----------------*/
    Fn_SymAdd( Vzadd , Vdserev / 2 , model->HSMHV2_vzadd0 , T2 ) ;
    Vzadd_ext_dVd = T2 / 2 ;
    if ( Vzadd < ps_conv ) {
      Vzadd = ps_conv ;
      Vzadd_ext_dVd = 0.0 ;
    ...
    here->HSMHV2_Vdserevz = Vdserevz ;
    here->HSMHV2_Vdserevz_dVd = Vdserevz_dVd ;
  ...
  if ( flg_rsrd == 1 || flg_rsrd == 3 ) {
    ...
    if ( here->HSMHV2_rdvd > 0.0 || here->HSMHV2_rsvd > 0.0 || pParam->HSMHV2_rdvg11 > 0.0 || pParam->HSMHV2_rdvb > 0.0 || here->HSMHV2subNode >= 0 ) {
      /*-----------------------------------------------------------*
       * Vxserevz: Modified bias introduced to realize symmetry at Vds=0.
       *-----------------*/
      Fn_SymAdd( Vzadd , Vdserev / 2 , model->HSMHV2_vzadd0 , T2 ) ;
	...
	Vzadd_ext_dVd = 0.0 ;
      ...
      if ( model->HSMHV2_cosym == 1 || VdseModeNML == 1 ) { /* HVMOS or normal mode LDMOS: */
      /* ... Vdse dependence             */
      T1     = VdseModeNML * here->HSMHV2_rd + VdseModeRVS * here->HSMHV2_rs ;
      T1_dT  = VdseModeNML * Rd0_dT         + VdseModeRVS * Rs0_dT ;
      T0     = VdseModeNML * here->HSMHV2_rdvd + VdseModeRVS * here->HSMHV2_rsvd ;
      ...
      /* ... Vgse dependence             */
      T10 = model->HSMHV2_rdvg12 + small ;
      T1     =   T4     * ( 1.0 + pParam->HSMHV2_rdvg11 * ( 1.0 - Vgserevz   / T10 ) ) ;
      T1_dVd =   T4_dVd * ( 1.0 + pParam->HSMHV2_rdvg11 * ( 1.0 - Vgserevz   / T10 ) )
	       + T4     *         pParam->HSMHV2_rdvg11 * (     - Vgserevz_dVd / T10 ) ;
      T1_dVg =   T4     *         pParam->HSMHV2_rdvg11 * (     - 1.0     ) / T10 ;
      T1_dT  =   T4_dT  * ( 1.0 + pParam->HSMHV2_rdvg11 * ( 1.0 - Vgserevz   / T10 ) ) ;
      Fn_SL2( T2 , T1 , T4 , rdsl2_dlt , T0 , T5 ) ;
      T2_dVd = T0 * T1_dVd + T5 * T4_dVd ;
      T2_dVg = T0 * T1_dVg ;
      T2_dT  = T0 * T1_dT  + T5 * T4_dT ;

      T3     = T4     * ( 1.0 + pParam->HSMHV2_rdvg11 ) ;
      T3_dVd = T4_dVd * ( 1.0 + pParam->HSMHV2_rdvg11 ) ;
      T3_dT  = T4_dT  * ( 1.0 + pParam->HSMHV2_rdvg11 ) ;
      ...
      /* ... Vbse dependence             */
      T1 = 1.0 - pParam->HSMHV2_rdvb * Vbserevz ;
      T1_dVb = - pParam->HSMHV2_rdvb ;
      T1_dVd = - pParam->HSMHV2_rdvb * Vbserevz_dVd ;
	...
	Rdrift_dT =    Rs0_dT ;
      ...
      /* Rsdrift */
      T4    = ( VdseModeNML * here->HSMHV2_rs + VdseModeRVS * here->HSMHV2_rd ) ;
      T4_dT = VdseModeNML * Rs0_dT         + VdseModeRVS * Rd0_dT ;
      T4_dVd = 0.0 ;

      if ( model->HSMHV2_cosym == 1 || VdseModeRVS == 1 ) { /* HVMOS or reverse mode LDMOS: */
	/* ... Vdse dependence             */
	T0     = VdseModeNML * here->HSMHV2_rsvd + VdseModeRVS * here->HSMHV2_rdvd ;
	T0_dT  = VdseModeNML * Rsvd_dT          + VdseModeRVS * Rdvd_dT ;
	T4 = T4 + T0 * ( 2.0 * model->HSMHV2_vzadd0 ) ; 
	T4_dT = T4_dT + T0_dT * ( 2.0 * model->HSMHV2_vzadd0 ) ;
	...
	Rsdrift_dT    = Rsdrift_dT    * T3 ;
      ...
      if ( here->HSMHV2subNode >= 0 && model->HSMHV2_cosym == 0 && 
         ( pParam->HSMHV2_nover * ( NSUBSUB + pParam->HSMHV2_nover ) ) > 0 ) {
	...
	T0 = NSUBSUB / ( pParam->HSMHV2_nover * ( NSUBSUB + pParam->HSMHV2_nover ) ) ;
	T4 = 2 * C_ESI / C_QE * T0 ;
	...
	if ( VdseModeNML == 1 ) { /* Vdse normal mode: */
      ...
      Rd = VdseModeNML * here->HSMHV2_rd + VdseModeRVS * here->HSMHV2_rs ;
      Rd_dT = VdseModeNML * Rd0_dT      + VdseModeRVS * Rs0_dT ;
      Rs = VdseModeNML * here->HSMHV2_rs + VdseModeRVS * here->HSMHV2_rd ;
    ...
    Rd += VdseModeNML * here->HSMHV2_rd0 + VdseModeRVS * here->HSMHV2_rs0 ;
    Rs += VdseModeNML * here->HSMHV2_rs0 + VdseModeRVS * here->HSMHV2_rd0 ;

    /* Re-stamps for hsmhvnoi.c */
    /* Please see hsmhvnoi.c */
    T0 = VdseModeNML * Rd + VdseModeRVS * Rs ; /* mode-dependent --> geometrical */
    if ( T0 > Res_min && model->HSMHV2_cothrml != 0 ) here->HSMHV2drainConductance = Mfactor / T0 ;
    else here->HSMHV2drainConductance = 0.0 ;
    T0 = VdseModeNML * Rs + VdseModeRVS * Rd ; /* mode-dependent --> geometrical */
    if ( T0 > Res_min && model->HSMHV2_cothrml != 0 ) here->HSMHV2sourceConductance = Mfactor / T0 ;
    ...
    T1 = Vbscl_dVbs * Vds / 2 ;
    Fn_SymAdd(  Vzadd , T1 , model->HSMHV2_vzadd0 , T2 ) ;
    ...
    T1 = here->HSMHV2_qnsub_esi * Cox0_inv * Cox0_inv ;
    T2 = Vgs - Vfb ;
    T3 = 1 + 2.0 / T1 * ( T2 - 1.0 / here->HSMHV2_betatnom - Vbscl ) ;
     ...
     * Quantum effect
     *-----------------*/
    if ( model->HSMHV2_flg_qme == 0 ) {
      flg_qme = 0 ;
    } else {
      flg_qme = 1 ;
    }

    T1 = here->HSMHV2_2qnsub_esi ;
    T2 = sqrt( T1 * Pb20 ) ;
    ...
    if ( flg_qme == 0 ) {
      ...
      T5     = Vgs  - Vbs - Vthq + model->HSMHV2_qme2 ;
      T5_dVb = -1.0 - Vthq_dVb ;
      T5_dVd =      - Vthq_dVd ;
      T5_dVg =  1.0 ;
      T5_dT =  0.0 ;
      ...
      dTox = model->HSMHV2_qme1 * T2 + model->HSMHV2_qme3 ;
      T7   = model->HSMHV2_qme1 ;
      dTox_dVb = T7 * T2_dVb ;
      dTox_dVd = T7 * T2_dVd ;
      dTox_dVg = T7 * T2_dVg ;
      dTox_dT = T7 * T2_dT ;
    ...
    Vthp = Pb20 + Vfb + Qb0 * Cox_inv + here->HSMHV2_ptovr;
    ...
    if( model->HSMHV2_codep ) {
       Pbsum = Pbsum + model->HSMHV2_depeta * Vdsz ;
       Pbsum_dVb = Pbsum_dVb + model->HSMHV2_depeta * Vdsz_dVbs ;
       Pbsum_dVd = Pbsum_dVd + model->HSMHV2_depeta * Vdsz_dVds ;
    }
     ...
     *-----------------*/
    if ( model->HSMHV2_lp != 0.0 ) {
      T1 = here->HSMHV2_2qnsub_esi ;
      T2 = model->HSMHV2_bs2 - Vbsz2 ;
      T3 = T2 + small ;
      ...
      T1 = 2.0 * C_QE * here->HSMHV2_nsubc * C_ESI ;
      ...
      T1 = C_ESI * Cox_inv ;
      T2 = here->HSMHV2_wdplp ;
      T4 = 1.0e0 / ( model->HSMHV2_lp * model->HSMHV2_lp ) ;
      T5 = 2.0e0 * ( model->HSMHV2_vbi - Pb20b ) * T1 * T2 * T4 ;
      dVth0 = T5 * sqrt_Pbsum ;
      T6 = 0.5 * T5 / sqrt_Pbsum ;
      T7 = 2.0e0 * ( model->HSMHV2_vbi - Pb20b ) * C_ESI * T2 * T4 * sqrt_Pbsum ;
      ...
      T2 = pParam->HSMHV2_scp1 + pParam->HSMHV2_scp3 * Pbsum / model->HSMHV2_lp ;
      T2_dVb = pParam->HSMHV2_scp3 * Pbsum_dVb / model->HSMHV2_lp ;
      T3 = T2 + pParam->HSMHV2_scp2 * Vdsz ;
      T3_dVb = T2_dVb + pParam->HSMHV2_scp2 * Vdsz_dVbs ;


      Vdx = model->HSMHV2_scp21 + Vdsz ;
      Vdx_dVbs = Vdsz_dVbs ;
      Vdx_dT = Vdsz_dT ;
      Vdx2 = Vdx * Vdx ;
      ...
      dVthLP = T1 * dVth0 * T3 + dqb - here->HSMHV2_msc / Vdx2 ;
      dVthLP_dVb = T1_dVb * dVth0 * T3 + T1 * dVth0_dVb * T3 +  T1 * dVth0 * T3_dVb 
                   + dqb_dVb + here->HSMHV2_msc / Vdx2 /Vdx2 *Vdx2_dVbs;
      T4 = T1 * dVth0 * pParam->HSMHV2_scp3 / model->HSMHV2_lp ;
      dVthLP_dVd = (Vthp_dVd - Vth0_dVd) * dVth0 * T3 + T1 * dVth0_dVd * T3 
                     + T4 * Pbsum_dVd
                     + T1 * dVth0 * pParam->HSMHV2_scp2 * Vdsz_dVds
                     + dqb_dVd
                 + 2.0e0 * here->HSMHV2_msc * Vdx * Vdsz_dVds / ( Vdx2 * Vdx2 ) ;
      ...
      dVthLP_dT = (Vthp_dT - Vth0_dT) * dVth0 * T3 + T1 * dVth0_dT * T3
                     + T4 * Pbsum_dT
                     + T1 * dVth0 * pParam->HSMHV2_scp2 * Vdsz_dT
                     + dqb_dT
                 + 2.0e0 * here->HSMHV2_msc * Vdx * Vdsz_dT / ( Vdx2 * Vdx2 );
    } else {
      dVthLP = 0.0e0 ;
      dVthLP_dVb = 0.0e0 ;
      dVthLP_dVd = 0.0e0 ;
    ...
    T1 = C_ESI * Cox_inv ;
    T2 = here->HSMHV2_wdpl ;
    T3 = here->HSMHV2_lgate - model->HSMHV2_parl2 ;
    T4 = 1.0e0 / ( T3 * T3 ) ;
    T5 = 2.0e0 * ( model->HSMHV2_vbi - Pb20b ) * T1 * T2 * T4 ;
    ...
    T1 = pParam->HSMHV2_sc3 / here->HSMHV2_lgate ;
    T4 = pParam->HSMHV2_sc1 + T1 * Pbsum ;
    T4_dVb = T1 * Pbsum_dVb ;
    T4_dVd = T1 * Pbsum_dVd ;
    T4_dVg = T1 * Pbsum_dVg ;
    T4_dT = T1 * Pbsum_dT ;

    T5 = T4 + pParam->HSMHV2_sc2 * Vdsz * ( 1.0 +  model->HSMHV2_sc4 * Pbsum );
    T5_dVb = T4_dVb + pParam->HSMHV2_sc2 * Vdsz * model->HSMHV2_sc4 * Pbsum_dVb 
             + pParam->HSMHV2_sc2 * Vdsz_dVbs * model->HSMHV2_sc4 * Pbsum;
    T5_dVd = T4_dVd + pParam->HSMHV2_sc2 * Vdsz_dVds * ( 1.0 + model->HSMHV2_sc4 * Pbsum )
             + pParam->HSMHV2_sc2 * Vdsz * model->HSMHV2_sc4 * Pbsum_dVd;
    T5_dVg = T4_dVg + pParam->HSMHV2_sc2 * Vdsz * model->HSMHV2_sc4 * Pbsum_dVg;
    T5_dT  = T4_dT  + pParam->HSMHV2_sc2 * Vdsz * model->HSMHV2_sc4 * Pbsum_dT
            + pParam->HSMHV2_sc2 * Vdsz_dT * model->HSMHV2_sc4 * Pbsum;
    ...
    T1 = 1.0 / Cox ;
    T2 = T1 * T1 ;
    T3 = 1.0 / ( Cox +  pParam->HSMHV2_wfc / Weff ) ;
    T4 = T3 * T3 ;
    T5 = T1 - T3 ;
    T6 = Qb0 * ( T2 - T4 ) ;

    dVthW = Qb0 * T5 + pParam->HSMHV2_wvth0 / here->HSMHV2_wg ;
     ...
     *-----------------*/
    dVth = dVthSC + dVthLP + dVthW + here->HSMHV2_dVthsm ;
     ...
     *-----------------*/
    T2 = sqrt( here->HSMHV2_2qnsub_esi * (Pb2-Vbsz) ) ;
    ...
    if ( here->HSMHV2_flg_pgd == 1 ) {
      ...
      T0 = here->HSMHV2_cnstpgd ;

      T3 = T7 - model->HSMHV2_pgd2 ;
      T3_dVb = T7_dVb ;
      T3_dVd = T7_dVd ;
      T3_dVg = T7_dVg ;
      T3_dT = T7_dT ;
     ...
     /* use analytical value in subthreshold region. */
      ...
      if ( flg_pprv >= 1 ) {
        ...
        *-----------------*/
        ...
        T2 = here->HSMHV2_ps0_dtemp_prv * Temp_dif ;
        if ( fabs( T1 + T2 ) < dP_max ) { Ps0_ini += T2 ; } 

        if ( flg_pprv == 2 ) {
          /* TX_dVxs = d^2 Ps0 / d Vxs^2 here */
          if ( Vbsc_dif2 > epsm10 ) {
            TX_dVbs = ( here->HSMHV2_ps0_dvbs_prv - here->HSMHV2_ps0_dvbs_prv2 )
                  / Vbsc_dif2 ;
          } else {
            TX_dVbs = 0.0 ;
          }
          if ( Vdsc_dif2 > epsm10 ) {
            TX_dVds = ( here->HSMHV2_ps0_dvds_prv - here->HSMHV2_ps0_dvds_prv2 )
                  / Vdsc_dif2 ;
          } else {
            TX_dVds = 0.0 ;
          }
          if ( Vgsc_dif2 > epsm10 ) {
            TX_dVgs = ( here->HSMHV2_ps0_dvgs_prv - here->HSMHV2_ps0_dvgs_prv2 )
                  / Vgsc_dif2 ;
          } else {
            TX_dVgs = 0.0 ;
          }
...
//      flg_conv = 1 ;
      ...
      *-----------------*/
      if ( flg_pprv >= 1  ) {
       ...
       /* self heating */
        T2 = here->HSMHV2_pds_dtemp_prv * Temp_dif ;
        if ( fabs( T1 + T2 ) < dP_max ) { Pds_ini += T2 ; }

        if ( flg_pprv == 2 ) {
          /* TX_dVxs = d^2 Pds / d Vxs^2 here */
          if ( Vbsc_dif2 > epsm10 ) {
            TX_dVbs = ( here->HSMHV2_pds_dvbs_prv - here->HSMHV2_pds_dvbs_prv2 )
                    / Vbsc_dif2 ;
          } else {
            TX_dVbs = 0.0 ;
          }
          if ( Vdsc_dif2 > epsm10 ) {
            TX_dVds = ( here->HSMHV2_pds_dvds_prv - here->HSMHV2_pds_dvds_prv2 )
                  / Vdsc_dif2 ;
          } else {
            TX_dVds = 0.0 ;
          }
          if ( Vgsc_dif2 > epsm10 ) {
            TX_dVgs = ( here->HSMHV2_pds_dvgs_prv - here->HSMHV2_pds_dvgs_prv2 )
                  / Vgsc_dif2 ;
          } else {
            TX_dVgs = 0.0 ;
          }
...
start_of_loopl: 
   ...
   /* if ( Pds < ps_conv ) { */
      ...
      *-----------------*/
      if( pParam->HSMHV2_clm2 < epsm10 && pParam->HSMHV2_clm3 < epsm10 ) {
        Lred = 0.0e0 ;
        Lred_dVbs = 0.0e0 ;
        Lred_dVds = 0.0e0 ;
        Lred_dVgs = 0.0e0 ;
        ...
        T1 = (1.0e0 - pParam->HSMHV2_clm1) ;
        Psdl = pParam->HSMHV2_clm1 * (Vds + Ps0) + T1 * Psl ;
        Psdl_dVbs = pParam->HSMHV2_clm1 * Ps0_dVbs + T1 * Psl_dVbs ;
        Psdl_dVds = pParam->HSMHV2_clm1 * (1.0 + Ps0_dVds) + T1 * Psl_dVds ;
        Psdl_dVgs = pParam->HSMHV2_clm1 * Ps0_dVgs + T1 * Psl_dVgs ;
        Psdl_dT = pParam->HSMHV2_clm1 * Ps0_dT + T1 * Psl_dT ;
     ...
     /* CLM5 & CLM6 */
      Lred *= here->HSMHV2_clmmod ;
      Lred_dVbs *= here->HSMHV2_clmmod ;
      Lred_dVds *= here->HSMHV2_clmmod ;
      Lred_dVgs *= here->HSMHV2_clmmod ;
      Lred_dT *= here->HSMHV2_clmmod ;
...
/*  end_of_mobility : */
      ...
      *-----------------*/
      ...
      betaWL_dT = here->HSMHV2_weff_nf * ( beta_inv_dT * Lch - beta_inv * Lch_dT ) / ( Lch * Lch ) ;
      ...
      * STI
      *-----------------*/
      if ( model->HSMHV2_coisti != 0 ) {
        /*---------------------------------------------------*
         * dVthSCSTI : Short-channel effect induced by Vds (STI).
         *-----------------*/      
        T1 = C_ESI * Cox_inv ;
        T2 = here->HSMHV2_wdpl ;
        T3 =  here->HSMHV2_lgatesm - model->HSMHV2_parl2 ;
        T4 = 1.0 / (T3 * T3) ;
        T5 = 2.0 * (model->HSMHV2_vbi - Pb20b) * T1 * T2 * T4 ;
      
        dVth0 = T5 * sqrt_Pbsum ;
        T6 = T5 * 0.5 / sqrt_Pbsum ;
        T7 = 2.0 * (model->HSMHV2_vbi - Pb20b) * C_ESI * T2 * T4 * sqrt_Pbsum ;
        ...
        T4 = pParam->HSMHV2_scsti1 ;
        T6 = pParam->HSMHV2_scsti2 ;
        T1  = T4 + T6 * Vdsz ;
        dVthSCSTI = dVth0 * T1 ;
        ...
        T1 = pParam->HSMHV2_vthsti - model->HSMHV2_vdsti * Vds ;
        T1_dVd = - model->HSMHV2_vdsti ;
        ...
        costi0 = here->HSMHV2_costi0 ;
        costi1 = here->HSMHV2_costi1 ;

        costi3 = here->HSMHV2_costi0_p2 * Cox_inv * Cox_inv ;
        T1 = 2.0 * here->HSMHV2_costi0_p2 * Cox_inv ;
        costi3_dVb = T1 * Cox_inv_dVb ;
        costi3_dVd = T1 * Cox_inv_dVd ;
        costi3_dVg = T1 * Cox_inv_dVg ;
        costi3_dT = 2 * here->HSMHV2_costi0 * here->HSMHV2_costi00 * 0.5 / sqrt(here->HSMHV2_beta_inv) * beta_inv_dT * Cox_inv * Cox_inv ; 
        T2 = 1.0 / costi3 ;
        ...
        T11 = beta * 0.25 ;
        T11_dT = beta_dT * 0.25 ;
        T10 = beta_inv - costi3 * T11 + Vfb - pParam->HSMHV2_vthsti - dVthSCSTI + small ;
        ...
        T3 = T10 + 0.5 * (T1 + T2) - Vfb + pParam->HSMHV2_vthsti + dVthSCSTI - Vbsz ;
        ...
        costi1_dT = 2 * here->HSMHV2_nin * Nin_dT * here->HSMHV2_nsti_p2 ;
        ...
        Qn0sti_dT = costi0 * (sq1sti_dT - sq2sti_dT) + here->HSMHV2_costi00 * 0.5 / sqrt( here->HSMHV2_beta_inv ) * beta_inv_dT * (sq1sti - sq2sti) ;
       ...
       /* T1: Vdsatsti */
        ...
        costi7 = 2.0 * here->HSMHV2_wsti * here->HSMHV2_nf * beta_inv ;
        costi7_dT = 2.0 * here->HSMHV2_wsti * here->HSMHV2_nf * beta_inv_dT ;
        T1 = Lch ;
      ...
      *----------------------*/
      if ( model->HSMHV2_coign != 0 && model->HSMHV2_cothrml != 0 && model->HSMHV2_codep==0 ) {
        kusai00 = VgVt * VgVt ;
 ...
 end_of_part_1:
    ...
    *----------------------*/
     ...
     *-----------------*/
    T1 =  ( Vds - Pds ) / 2 ;
    Fn_SymAdd( Pzadd , T1 , model->HSMHV2_pzadd0 , T2 ) ;
    T2 /= 2 ;
    ...
    Ids += Weff/Leff * model->HSMHV2_gdsleak * Vds ; 
    Ids_dVds += Weff/Leff * model->HSMHV2_gdsleak ;
  ...
  if ( model->HSMHV2_coisub == 0 ) {
    ...
    * Conductive case. 
    *-----------------*/
    if ( pParam->HSMHV2_sub1 > 0.0e0 && pParam->HSMHV2_vmax > 0.0e0 ) {
      T0 = here->HSMHV2_vg2const ;
      T1 = T0 * Vgp ; 
      T1_dVd = T0 * Vgp_dVds ;
      T1_dVg = T0 * Vgp_dVgs ;
      T1_dVb = T0 * Vgp_dVbs ;
      ...
      T2 = here->HSMHV2_lgate / (here->HSMHV2_xgate + here->HSMHV2_lgate) ;

      Psisubsat = pParam->HSMHV2_svds * Vdsz + Ps0z - T2 * Psislsat ;      
      Psisubsat_dVd = pParam->HSMHV2_svds * Vdsz_dVds + Ps0z_dVds - T2 * Psislsat_dVd ; 
      Psisubsat_dVg = Ps0z_dVgs - T2 * Psislsat_dVg ; 
      Psisubsat_dVb =  pParam->HSMHV2_svds * Vdsz_dVbs + Ps0z_dVbs - T2 * Psislsat_dVb ;
      Psisubsat_dT  =  pParam->HSMHV2_svds * Vdsz_dT   + Ps0z_dT   - T2 * Psislsat_dT  ;
      ...
      T5 = here->HSMHV2_xsub1 ;
      T6 = here->HSMHV2_xsub2 ;
      T2 = exp( - T6 / Psisubsat ) ;
      ...
      Isub_dVbs = Isub_dVds = Isub_dVgs = 0.0e0 ;
      Isub_dT  = 0.0e0 ;
    } /* end of if ( pParam->HSMHV2_sub1 ... ) else block. */

    /*---------------------------------------------------*
     * Impact-Ionization Current in the Drift Region
     *-----------------*/
    T8 = here->HSMHV2_subld1 ;
    if ( T8 != 0.0 ) {
      T0 = Vddp ;
      T0_dVb = 0.0e0 ;
      T0_dVd = 0.0e0 ;
      ...
      T4 = T0 - model->HSMHV2_xpvdth * ( 1.0 + model->HSMHV2_xpvdthg * Vgs ) ;
      T4_dVb = T0_dVb ;
      T4_dVd = T0_dVd ;
      T4_dVg = T0_dVg - model->HSMHV2_xpvdth * model->HSMHV2_xpvdthg ;
      T4_dT = T0_dT ;
      T4_dVddp = T0_dVddp ;
      ...
      T10 = here->HSMHV2_xpdv * model->HSMHV2_xldld * exp( - 1.0e0 / T4 ) ;
      T11 = T10 * ( 1.0 + 1.0 / T4 ) ;
      T3 = T4 * T10 ;
      T3_dVb = T4_dVb * T11 ;
      T3_dVd = T4_dVd * T11 ;
      ...
      T4 = 1.0 / ( T0 * T1 ) ;
      T7 = Ldrift0 * modelMKS->HSMHV2_subld2 ;
      T2= exp( - T7 * T4 );
      T6 = T7 * T2 * T4 * T4 ;
     ...
     *-----------------*/
  if ( (Isub + IsubLD) > 0e0 && here->HSMHV2_ibpc1 != 0e0 && model->HSMHV2_codep==0 ) {

    /* delta Vbs */
    T0 = 1e0 + pParam->HSMHV2_ibpc2 * dVth ;
    T1 = Isub + IsubLD ;
    dVbsIBPC = here->HSMHV2_ibpc1 * T0 * T1 ;
    dVbsIBPC_dVbs = here->HSMHV2_ibpc1 * ( pParam->HSMHV2_ibpc2 * dVth_dVb * T1 + T0 * ( Isub_dVbs + IsubLD_dVbs ) ) ;
    dVbsIBPC_dVds = here->HSMHV2_ibpc1 * ( pParam->HSMHV2_ibpc2 * dVth_dVd * T1 + T0 * ( Isub_dVds + IsubLD_dVds ) ) ;
    dVbsIBPC_dVgs = here->HSMHV2_ibpc1 * ( pParam->HSMHV2_ibpc2 * dVth_dVg * T1 + T0 * ( Isub_dVgs + IsubLD_dVgs ) ) ;
    dVbsIBPC_dT   = here->HSMHV2_ibpc1 * ( pParam->HSMHV2_ibpc2 * dVth_dT  * T1 + T0 * ( Isub_dT   + IsubLD_dT ) ) ;
    dVbsIBPC_dVddp = here->HSMHV2_ibpc1 * T0 * IsubLD_dVddp ;
  ...
  T3 = 1 / TTEMP ;
  T0 =- model->HSMHV2_igtemp2 * T3 * T3   
          - 2 * model->HSMHV2_igtemp3 * T3 * T3 * T3   ;
  ...
  if ( model->HSMHV2_coiigs != 0 ) {
    ...
    if ( flg_noqi == 0 ) {
      ...
      T1 = Vgsz - Vfb + modelMKS->HSMHV2_gleak4 * (dVth - dPpg) * Leff - Psdlz * pParam->HSMHV2_gleak3 ;
      T3 = modelMKS->HSMHV2_gleak4 * Leff ;
      T1_dVg = Vgsz_dVgs + T3 * (dVth_dVg - dPpg_dVg) - Psdlz_dVgs * pParam->HSMHV2_gleak3 ;
      T1_dVd = Vgsz_dVds + T3 * (dVth_dVd - dPpg_dVd)  - Psdlz_dVds * pParam->HSMHV2_gleak3 ;
      T1_dVb = Vgsz_dVbs + T3 * ( dVth_dVb - dPpg_dVb )  - Psdlz_dVbs * pParam->HSMHV2_gleak3 ;
      T1_dT  = Vgsz_dT   + T3 * ( dVth_dT - dPpg_dT )  - Psdlz_dT * pParam->HSMHV2_gleak3 ;
      ...
      T0 = Leff * here->HSMHV2_weff_nf ;
      T7 = modelMKS->HSMHV2_gleak7 / (modelMKS->HSMHV2_gleak7 + T0) ;
      
      T6 = pParam->HSMHV2_gleak6 ;
      T9 = T6 / (T6 + Vdsz) ;
      T9_dVb = - T9 / (T6 + Vdsz) * Vdsz_dVbs ;
      T9_dVd = - T9 / (T6 + Vdsz) * Vdsz_dVds ;
      
      T4 = 1 / (Etun + small ) ;
      T1 = - pParam->HSMHV2_gleak2 * Egp32 * T4 ;
      T3 =  pParam->HSMHV2_gleak2 * T4 * T4;
      ...
      if ( T1 < - EXP_THR ) {
        ...
        T3 = pParam->HSMHV2_gleak1 / Egp12 * C_QE * T0 ;
        T3_dT = - Egp12_dT * pParam->HSMHV2_gleak1 / Egp12 / Egp12 * C_QE * T0 ;
    ...
    /* Igs */
      T0 = - pParam->HSMHV2_glksd2 * Vgs + modelMKS->HSMHV2_glksd3 ;
      T2 = exp (Tox0 * T0);
      T2_dVg = (- Tox0 * pParam->HSMHV2_glksd2) * T2;
        
      T0 = Vgs / Tox0 / Tox0 ;
      T3 = Vgs * T0 ;
      T3_dVg = 2.0 * T0 * (1.0 ) ;
      T4 = pParam->HSMHV2_glksd1 / 1.0e6 * here->HSMHV2_weff_nf ;
      Igs = T4 * T2 * T3 ;
    ...
    /* Igd */
      T1 = Vgs - Vds ;
      T0 = - pParam->HSMHV2_glksd2 * T1 + modelMKS->HSMHV2_glksd3 ;
      T2 = exp (Tox0 * T0);
      T2_dVg = (- Tox0 * pParam->HSMHV2_glksd2) * T2;
      T2_dVd = (+ Tox0 * pParam->HSMHV2_glksd2) * T2;
      T2_dVb = 0.0 ;
    ...
    /* Igb */
      Etun = ( - ( Vgs - Vbs ) + Vfb + model->HSMHV2_glkb3 ) / Tox0 ;
      Etun_dVgs = - 1.0 / Tox0 ;
      Etun_dVds = 0.0 ;
      Etun_dVbs =   1.0 / Tox0 ;
      ...
      T1 = - pParam->HSMHV2_glkb2 / Etun ;
      if ( T1 < - EXP_THR ) {
        Igb = 0.0 ;
        Igb_dVgs = Igb_dVds = Igb_dVbs = Igb_dT = 0.0 ;
      } else {
        T2 = exp ( T1 );
        T3 =  pParam->HSMHV2_glkb2 / ( Etun * Etun ) * T2 ;
        T2_dVg = T3 * Etun_dVgs ;
	T2_dVb = T3 * Etun_dVbs ;
          
        T3 = pParam->HSMHV2_glkb1 * here->HSMHV2_weff_nf * Leff ;
        Igb = T3 * Etun * Etun * T2 ;
	...
	Igb_dT = 0.0;
      ...
      Eg12 = here->HSMHV2_sqrt_eg ;
      Eg32 = here->HSMHV2_eg * Eg12 ;
      T2 = - ( pParam->HSMHV2_fvbs * Vbsz - Vgsz + dVthSC + dVthLP - pParam->HSMHV2_fn3 ) / Tox0 ;
      T2_dVd = - ( pParam->HSMHV2_fvbs * Vbsz_dVds - Vgsz_dVds + dVthSC_dVd + dVthLP_dVd 
                   ) / Tox0 ;
      T2_dVg = - ( - Vgsz_dVgs + dVthSC_dVg + dVthLP_dVg 
                   ) / Tox0 ;
      T2_dVb = - ( pParam->HSMHV2_fvbs * Vbsz_dVbs -Vgsz_dVbs + dVthSC_dVb + dVthLP_dVb 
                   ) / Tox0 ;
      T2_dT = - (  pParam->HSMHV2_fvbs * Vbsz_dT   -Vgsz_dT   + dVthSC_dT + dVthLP_dT 
                   ) / Tox0 ;

      T0 = T2 * T2 ;
      T1 = pParam->HSMHV2_fn2 * Eg32 ;
      T1_dT = 1.5 * Eg_dT * pParam->HSMHV2_fn2 * Eg12 ;
      T3 = - T1 / T2  ;
      if ( T3 < - EXP_THR ) {
        T5 = 0.0 ;
      ...
      T4 = C_QE * pParam->HSMHV2_fn1 * here->HSMHV2_weff_nf * here->HSMHV2_lgate / Eg12 ;  
      T4_dT = (- 0.5) * Eg_dT * T4 / here->HSMHV2_eg ;  
      if ( 2e0 * T2 + T1 < 0e0 ){
  ...
  if( model->HSMHV2_cogidl == 0 ){
    ...
    T1 = model->HSMHV2_gidl3 * (Vdsp + model->HSMHV2_gidl4) - Vgs + (dVthSC + dVthLP) * model->HSMHV2_gidl5 ;
    T1_dT = (dVthSC_dT + dVthLP_dT) * model->HSMHV2_gidl5 ;
    T2 = 1.0 / Tox0 ;
    E1 = T1 * T2 ;
    E1_dVb = ((model->HSMHV2_gidl5 * (dVthSC_dVb + dVthLP_dVb)) ) * T2 ;
    E1_dVd = ((model->HSMHV2_gidl3 * Vdsp_dVd) + model->HSMHV2_gidl5 * (dVthSC_dVd + dVthLP_dVd)) * T2 ;
    E1_dVg = (-1.0 + model->HSMHV2_gidl5 * (dVthSC_dVg + dVthLP_dVg) ) * T2 ;
    E1_dT = T1_dT * T2 ;
    ...
    T3 = 1 / (Egidl + small) ;
    T0 = - pParam->HSMHV2_gidl2 * Egp32 * T3 ;
    T0_dT = - pParam->HSMHV2_gidl2 * T3 *( Egp32_dT - Egidl_dT * T3 * Egp32 )  ;
    if ( T0 < - EXP_THR ) {
      Igidl = 0.0 ;
      ...
      T1 = exp ( T0 ) ;
      T1_dT = T0_dT * T1 ;
      T2 = pParam->HSMHV2_gidl1 / Egp12 * C_QE * here->HSMHV2_weff_nf ;
      T2_dT = - Egp12_dT * pParam->HSMHV2_gidl1 / Egp12 / Egp12 * C_QE * here->HSMHV2_weff_nf ;
      Igidl = T2 * Egidl * Egidl * T1 ;
      T3 = T2 * T1 * Egidl * (2.0 + pParam->HSMHV2_gidl2 * Egp32 * Egidl / (Egidl + small) / (Egidl + small)) ;
  ...
  if( model->HSMHV2_cogidl == 0){
    ...
    T1 = model->HSMHV2_gidl3 * ( - Vdsp + model->HSMHV2_gidl4 )
      - ( Vgs - Vdsp ) + ( dVthSC + dVthLP ) * model->HSMHV2_gidl5 ;

    T1_dT = ( dVthSC_dT + dVthLP_dT ) * model->HSMHV2_gidl5 ;
    T2 = 1.0 / Tox0 ;
    E1 = T1 * T2 ;
    E1_dVb = ((model->HSMHV2_gidl5 * (dVthSC_dVb + dVthLP_dVb)) ) * T2 ;
    E1_dVd = (((1.0-model->HSMHV2_gidl3 ) * Vdsp_dVd) + model->HSMHV2_gidl5 * (dVthSC_dVd + dVthLP_dVd)) * T2 ;
    E1_dVg = (-1.0 + model->HSMHV2_gidl5 * (dVthSC_dVg + dVthLP_dVg) ) * T2 ;
    E1_dT = T1_dT * T2 ;
    ...
    T3 =  1  / (Egisl + small) ;
    T0 = - pParam->HSMHV2_gidl2 * Egp32 * T3 ;
    T0_dT = - pParam->HSMHV2_gidl2 * T3 * ( Egp32_dT - Egisl_dT * T3 * Egp32 )  ;
    if ( T0 < - EXP_THR ) {
      Igisl = 0.0 ;
      ...
      T1_dT = T0_dT * T1 ;
      T3 = 1 / Egp12 ;
      T2 = pParam->HSMHV2_gidl1 * T3 * C_QE * here->HSMHV2_weff_nf ;
      T2_dT = - pParam->HSMHV2_gidl1 * Egp12_dT * T3 * T3 * C_QE * here->HSMHV2_weff_nf ;
      Igisl = T2 * Egisl * Egisl * T1 ;
      T3 = T2 * T1 * Egisl * (2.0 + pParam->HSMHV2_gidl2 * Egp32 * Egisl / (Egisl + small) / (Egisl + small)) ;
...
/* end_of_part_2: */
   ...
   * Overlap charges
   *-----------------*/
  Coovlps = (int)ModeNML * model->HSMHV2_coovlps + (int)ModeRVS * model->HSMHV2_coovlp ;
  Coovlpd = (int)ModeRVS * model->HSMHV2_coovlps + (int)ModeNML * model->HSMHV2_coovlp ;
  Lovers = ModeNML * here->HSMHV2_lovers + ModeRVS * here->HSMHV2_loverld ;
  Loverd = ModeRVS * here->HSMHV2_lovers + ModeNML * here->HSMHV2_loverld ;
  Novers = ModeNML * pParam->HSMHV2_novers + ModeRVS * pParam->HSMHV2_nover ;
  Noverd = ModeRVS * pParam->HSMHV2_novers + ModeNML * pParam->HSMHV2_nover ;
  CVDSOVER = pParam->HSMHV2_cvdsover ;
  ...
  *-----------------*/
  if ( Coovlps == 0 ) {
    flg_overgiven = ( (int)ModeNML * model->HSMHV2_cgso_Given 
                    + (int)ModeRVS * model->HSMHV2_cgdo_Given  ) ;
    if ( flg_overgiven ) {
      Cgso = ModeNML * pParam->HSMHV2_cgso + ModeRVS * pParam->HSMHV2_cgdo ;
      Cgso *= - here->HSMHV2_weffcv_nf ;
    } else {
      Cgso = - Cox0 * Lovers * here->HSMHV2_weffcv_nf ;
    }
   ...
   *-----------------*/
    ...
    if ( Lovers > 0.0 && Novers == 0.0 ){
      cov_slp = modelMKS->HSMHV2_ovslp ;
      cov_mag = model->HSMHV2_ovmag ;
      T1 = Cox0 * here->HSMHV2_weffcv_nf ;
...
#include "hsmhv2eval_qover.h"
    ...
    if (here->HSMHV2_mode != HiSIM_NORMAL_MODE) {
    here->HSMHV2_Ps0LD      = Ps0LD ;
    here->HSMHV2_Ps0LD_dVds = - Ps0LD_dVds - Ps0LD_dVgs - Ps0LD_dVbs;
    here->HSMHV2_Ps0LD_dVgs = Ps0LD_dVgs ;
    here->HSMHV2_Ps0LD_dVbs = Ps0LD_dVbs ;
    here->HSMHV2_Ps0LD_dTi  = Ps0LD_dT ;
    here->HSMHV2_QbuLD      = QbuLD ;
    here->HSMHV2_QbuLD_dVds = - QbuLD_dVds - QbuLD_dVgs - QbuLD_dVbs;
    here->HSMHV2_QbuLD_dVgs = QbuLD_dVgs ;
    here->HSMHV2_QbuLD_dVbs = QbuLD_dVbs ;
    here->HSMHV2_QbuLD_dTi  = QbuLD_dT ;
    QosAD = - model->HSMHV2_qovadd * ( Vds - Pds ) ;
    T3    =   model->HSMHV2_qovadd ;
    ...
    T4 = here->HSMHV2_weffcv_nf * Lovers * ( 1 - CVDSOVER ) ;
...
#include "hsmhv2eval_qover.h"
  ...
  *-----------------*/
  if ( Coovlpd == 0 ) {
    flg_overgiven = ( (int)ModeRVS * model->HSMHV2_cgso_Given 
                    + (int)ModeNML * model->HSMHV2_cgdo_Given  ) ;
    if ( flg_overgiven ) {
      Cgdo = ModeRVS * pParam->HSMHV2_cgso + ModeNML * pParam->HSMHV2_cgdo ;
      Cgdo *= - here->HSMHV2_weffcv_nf ;
    } else {
      Cgdo = - Cox0 * Loverd * here->HSMHV2_weffcv_nf ;
    }
...
#include "hsmhv2eval_qover.h"
    ...
    if (here->HSMHV2_mode == HiSIM_NORMAL_MODE) {
    here->HSMHV2_Ps0LD      = Ps0LD ;
    here->HSMHV2_Ps0LD_dVds = Ps0LD_dVds ;
    here->HSMHV2_Ps0LD_dVgs = Ps0LD_dVgs ;
    here->HSMHV2_Ps0LD_dVbs = Ps0LD_dVbs ;
    here->HSMHV2_Ps0LD_dTi  = Ps0LD_dT ;
    here->HSMHV2_QbuLD      = QbuLD ;
    here->HSMHV2_QbuLD_dVds = QbuLD_dVds ;
    here->HSMHV2_QbuLD_dVgs = QbuLD_dVgs ;
    here->HSMHV2_QbuLD_dVbs = QbuLD_dVbs ;
    here->HSMHV2_QbuLD_dTi  = QbuLD_dT ;
    QodAD =   model->HSMHV2_qovadd * ( Vds - Pds ) ;
    T3    = - model->HSMHV2_qovadd ;
    ...
    T4 = here->HSMHV2_weffcv_nf * Loverd * ( 1 - CVDSOVER ) ;
...
#include "hsmhv2eval_qover.h"
   ...
   *-----------------*/
  Cgbo_loc = - pParam->HSMHV2_cgbo * here->HSMHV2_lgate ;
  ...
  if ( model->HSMHV2_xqy == 0 ){
    ...
    T10 = here->HSMHV2_wdpl ;
    T3 = T10 * 1.3 ;
    T2 = C_ESI * here->HSMHV2_weffcv_nf * T3 ;
    Qy =  - ( ( Ps0 + Vds  - T1 ) / model->HSMHV2_xqy ) * T2 ;
    Qy_dVds =  - ( ( Ps0_dVds + 1.0e0 - T1_dVd ) / model->HSMHV2_xqy ) * T2 ;
    Qy_dVgs =  - ( ( Ps0_dVgs  - T1_dVg ) / model->HSMHV2_xqy ) * T2 ;
    Qy_dVbs =  - ( ( Ps0_dVbs  - T1_dVb ) / model->HSMHV2_xqy ) * T2 ;
    Qy_dT =  - ( ( Ps0_dT - T1_dT ) / model->HSMHV2_xqy ) * T2 ; 
  }

  if ( model->HSMHV2_xqy1 != 0.0 ){
    Qy += here->HSMHV2_cqyb0 * Vbs ;
    Qy_dVbs += here->HSMHV2_cqyb0 ;
  }

  /*---------------------------------------------------* 
   * Fringing capacitance.
   *-----------------*/ 
  Cfd = here->HSMHV2_cfrng ;
  Cfs = here->HSMHV2_cfrng ;
  Qfd = Cfd * ( Vgse - Vdse ) ;
  Qfs = Cfs * Vgse ;
  ...
  if (flg_nqs) {
    if(ckt->CKTmode & MODETRAN){
      if( ckt->CKTmode & MODEINITTRAN ){
        ...
        if (flg_noqi == 0) {
          T12 = model->HSMHV2_dly1;
          T10 = model->HSMHV2_dly2;
...
/* end_of_part_5: */
   ...
   * 1/f noise.
   *-----------------*/
  if ( model->HSMHV2_coflick != 0 && !flg_noqi ) {
    
    NFalp = pParam->HSMHV2_nfalp ;
    NFtrp = pParam->HSMHV2_nftrp ;
    Cit = modelMKS->HSMHV2_cit ;

    T1 = Qn0 / C_QE ;
    T2 = ( Cox + Qn0 / ( Ps0 - Vbscl ) + Cit ) * beta_inv / C_QE ;
    T3 = -2.0E0 * Qi_noi / C_QE / Lch / here->HSMHV2_weff_nf - T1 ;
    if ( T3 != T1 ) {
    ...
    Nflic = Ids * Ids * NFtrp / ( Lch * beta * here->HSMHV2_weff_nf ) * T4 ;
  } else {
    Nflic = 0.0 ;
  }
   ...
   * thermal noise.
   *-----------------*/
  if ( model->HSMHV2_cothrml != 0 && !flg_noqi && model->HSMHV2_codep==0 ) {
    ...
    if ( 1.0e0 - epsm10 <= model->HSMHV2_bb && model->HSMHV2_bb <= 1.0e0 + epsm10 ) {
      ...
      T7  = Fn_Pow( Eyd, model->HSMHV2_bb - 1.0e0 ) ;
    }
    T8 = T12 * T7 ;
    T9 = 1.0e0 + T8 ;
    T10 = Fn_Pow( T9, ( - 1.0e0 / model->HSMHV2_bb - 1.0e0 ) ) ;
    T11 = T9 * T10 ;
    Mud_hoso = Muun * T11 ;
    Mu_Ave = ( Mu + Mud_hoso ) / 2.0 ;
    
    /* Sid_h = GAMMA * 4.0 * C_KB * model->HSMHV2_temp * gds0_h2; */
    T0 = Alpha * Alpha ;
    Nthrml  = here->HSMHV2_weff_nf * Cox * VgVt * Mu
   ...
   *----------------------*/
  if ( model->HSMHV2_coign != 0 && model->HSMHV2_cothrml != 0 && flg_ign == 1 && !flg_noqi && model->HSMHV2_codep==0) {
    ...
    gds0_ign = here->HSMHV2_weff_nf / Lch * Mu * Cox ;
...
/* end_of_part_6: */
   ...
   *-----------------*/
  here->HSMHV2_ids  = Mfactor * Ids ;
  here->HSMHV2_dIds_dVdsi = Mfactor * Ids_dVds ;
  here->HSMHV2_dIds_dVgsi = Mfactor * Ids_dVgs ;
  here->HSMHV2_dIds_dVbsi = Mfactor * Ids_dVbs ;
  here->HSMHV2_dIds_dTi   = Mfactor * Ids_dT  ;

  here->HSMHV2_idsibpc  = Mfactor * IdsIBPC ;
  here->HSMHV2_dIdsIBPC_dVdsi = Mfactor * IdsIBPC_dVds ;
  here->HSMHV2_dIdsIBPC_dVgsi = Mfactor * IdsIBPC_dVgs ;
  here->HSMHV2_dIdsIBPC_dVbsi = Mfactor * IdsIBPC_dVbs ;
  here->HSMHV2_dIdsIBPC_dTi   = Mfactor * IdsIBPC_dT  ;
  here->HSMHV2_dIdsIBPC_dVddp = Mfactor * IdsIBPC_dVddp  ;
  ...
  if (flg_nqs) { /* for flat handling of NQS: the NQS charges are added in hsmhvld */
    ...
    here->HSMHV2_dqdp_dVdse = 0.0 ;
    here->HSMHV2_dqdp_dVgse = 0.0 ;
    here->HSMHV2_dqdp_dVbse = 0.0 ;
    here->HSMHV2_dqdp_dTi   = 0.0 ;
    here->HSMHV2_dqsp_dVdse = 0.0 ;
    here->HSMHV2_dqsp_dVgse = 0.0 ;
    here->HSMHV2_dqsp_dVbse = 0.0 ;
    here->HSMHV2_dqsp_dTi   = 0.0 ;

    here->HSMHV2_dQdi_dVdsi = 0.0 ;
    here->HSMHV2_dQdi_dVgsi = 0.0 ;
    here->HSMHV2_dQdi_dVbsi = 0.0 ;
    here->HSMHV2_dQdi_dTi   = 0.0 ;
    here->HSMHV2_dQg_dVdsi  = 0.0 ;
    here->HSMHV2_dQg_dVgsi  = 0.0 ;
    here->HSMHV2_dQg_dVbsi  = 0.0 ;
    here->HSMHV2_dQg_dTi    = 0.0 ;
    here->HSMHV2_dQb_dVdsi  = 0.0 ;
    here->HSMHV2_dQb_dVgsi  = 0.0 ;
    here->HSMHV2_dQb_dVbsi  = 0.0 ;
    here->HSMHV2_dQb_dTi    = 0.0 ;

    here->HSMHV2_qgext = 0.0 ;
    here->HSMHV2_qdext = 0.0 ;
    here->HSMHV2_qsext = 0.0 ;
    
    here->HSMHV2_dQdext_dVdse = 0.0 ;
    here->HSMHV2_dQdext_dVgse = 0.0 ;
    here->HSMHV2_dQdext_dVbse = 0.0 ;
    here->HSMHV2_dQdext_dTi   = 0.0 ;
    here->HSMHV2_dQgext_dVdse = 0.0 ;
    here->HSMHV2_dQgext_dVgse = 0.0 ;
    here->HSMHV2_dQgext_dVbse = 0.0 ;
    here->HSMHV2_dQgext_dTi   = 0.0 ;
    here->HSMHV2_dQbext_dVdse = 0.0 ;
    here->HSMHV2_dQbext_dVgse = 0.0 ;
    here->HSMHV2_dQbext_dVbse = 0.0 ;
    here->HSMHV2_dQbext_dTi   = 0.0 ;
    here->HSMHV2_tau       = tau ;
    here->HSMHV2_tau_dVgsi = tau_dVgs ;
    here->HSMHV2_tau_dVdsi = tau_dVds ;
    here->HSMHV2_tau_dVbsi = tau_dVbs ;
    here->HSMHV2_tau_dTi   = tau_dT   ;
      
    here->HSMHV2_taub       = taub ;
    here->HSMHV2_taub_dVgsi = taub_dVgs ;
    here->HSMHV2_taub_dVdsi = taub_dVds ;
    here->HSMHV2_taub_dVbsi = taub_dVbs ;
    here->HSMHV2_taub_dTi   = taub_dT   ;
      
    here->HSMHV2_Xd       = Qdrat;
    here->HSMHV2_Xd_dVgsi = Qdrat_dVgs ;
    here->HSMHV2_Xd_dVdsi = Qdrat_dVds ;
    here->HSMHV2_Xd_dVbsi = Qdrat_dVbs ;
    here->HSMHV2_Xd_dTi   = Qdrat_dT   ;

    here->HSMHV2_Qbulk       = Mfactor * Qb ;
    here->HSMHV2_Qbulk_dVgsi = Mfactor * Qb_dVgs ;
    here->HSMHV2_Qbulk_dVdsi = Mfactor * Qb_dVds ;
    here->HSMHV2_Qbulk_dVbsi = Mfactor * Qb_dVbs ;
    here->HSMHV2_Qbulk_dTi   = Mfactor * Qb_dT  ;
    
    here->HSMHV2_Qi       = Mfactor * Qi ;
    here->HSMHV2_Qi_dVgsi = Mfactor * Qi_dVgs ;
    here->HSMHV2_Qi_dVdsi = Mfactor * Qi_dVds ;
    here->HSMHV2_Qi_dVbsi = Mfactor * Qi_dVbs ;
    here->HSMHV2_Qi_dTi   = Mfactor * Qi_dT  ;

  } else  { /* QS */

    here->HSMHV2_qg = Mfactor * - (Qb + Qi) ;
    here->HSMHV2_qd = Mfactor * Qd ;
    here->HSMHV2_qs = Mfactor * ( Qi - Qd ) ;
    here->HSMHV2_qdp = 0.0 ;
    here->HSMHV2_qsp = 0.0 ;
    ...
    here->HSMHV2_dQbext_dVbse = 0.0 ;
    here->HSMHV2_dQbext_dTi   = 0.0 ;
    here->HSMHV2_dQdi_dVdsi = Mfactor * Qd_dVds ;
    here->HSMHV2_dQdi_dVgsi = Mfactor * Qd_dVgs ;
    here->HSMHV2_dQdi_dVbsi = Mfactor * Qd_dVbs ;
    here->HSMHV2_dQdi_dTi   = Mfactor * Qd_dT  ;
    here->HSMHV2_dQg_dVdsi  = Mfactor * ( - Qb_dVds - Qi_dVds ) ;
    here->HSMHV2_dQg_dVgsi  = Mfactor * ( - Qb_dVgs - Qi_dVgs ) ;
    here->HSMHV2_dQg_dVbsi  = Mfactor * ( - Qb_dVbs - Qi_dVbs ) ;
    here->HSMHV2_dQg_dTi    = Mfactor * ( - Qb_dT  - Qi_dT  ) ;
    here->HSMHV2_dQb_dVdsi  = Mfactor * Qb_dVds ;
    here->HSMHV2_dQb_dVgsi  = Mfactor * Qb_dVgs ;
    here->HSMHV2_dQb_dVbsi  = Mfactor * Qb_dVbs ;
    here->HSMHV2_dQb_dTi    = Mfactor * Qb_dT  ;
  }
   ...
   *-----------------*/
  if ( model->HSMHV2_coadov == 1 ) {
      here->HSMHV2_qg += Mfactor * ( Qgod + Qgos + Qgbo + Qy - Qovd - Qovs ) ;
      here->HSMHV2_qd += Mfactor * ( - Qgod - Qy + QbdLD ) ;
      here->HSMHV2_qs += Mfactor * ( - Qgos      + QbsLD ) ;

      here->HSMHV2_qdp += Mfactor * ( - Qfd - Qgdo ) ;
      here->HSMHV2_qsp += Mfactor * ( - Qfs - Qgso ) ;

      here->HSMHV2_cddo        = Mfactor * ( - Qgod_dVds - Qy_dVds + QbdLD_dVds ) ;
      here->HSMHV2_dQdi_dVdsi +=     here->HSMHV2_cddo ;
      here->HSMHV2_cdgo        = Mfactor * ( - Qgod_dVgs - Qy_dVgs + QbdLD_dVgs ) ;
      here->HSMHV2_dQdi_dVgsi +=     here->HSMHV2_cdgo ;
      here->HSMHV2_cdbo        = Mfactor * ( - Qgod_dVbs - Qy_dVbs + QbdLD_dVbs ) ;
      here->HSMHV2_dQdi_dVbsi +=     here->HSMHV2_cdbo ;
      here->HSMHV2_dQdi_dTi   += Mfactor * ( - Qgod_dT  - Qy_dT  + QbdLD_dT  ) ;
      here->HSMHV2_cgdo        = Mfactor * (   Qgod_dVds + Qgos_dVds + Qgbo_dVds + Qy_dVds - Qovd_dVds - Qovs_dVds  ) ;
      here->HSMHV2_dQg_dVdsi  +=     here->HSMHV2_cgdo ;
      here->HSMHV2_cggo        = Mfactor * (   Qgod_dVgs + Qgos_dVgs + Qgbo_dVgs + Qy_dVgs - Qovd_dVgs - Qovs_dVgs  ) ;
      here->HSMHV2_dQg_dVgsi  +=     here->HSMHV2_cggo ;
      here->HSMHV2_cgbo        = Mfactor * (   Qgod_dVbs + Qgos_dVbs + Qgbo_dVbs + Qy_dVbs - Qovd_dVbs - Qovs_dVbs  ) ;
      here->HSMHV2_dQg_dVbsi  +=     here->HSMHV2_cgbo ;
      here->HSMHV2_dQg_dTi    += Mfactor * (   Qgod_dT  + Qgos_dT  + Qgbo_dT  + Qy_dT  - Qovd_dT  - Qovs_dT   ) ;
      here->HSMHV2_cbdo        = Mfactor * ( - Qgbo_dVds + QidLD_dVds + QisLD_dVds ) ;
      here->HSMHV2_dQb_dVdsi  +=     here->HSMHV2_cbdo ;
      here->HSMHV2_cbgo        = Mfactor * ( - Qgbo_dVgs + QidLD_dVgs + QisLD_dVgs ) ;
      here->HSMHV2_dQb_dVgsi  +=     here->HSMHV2_cbgo ;
      here->HSMHV2_cbbo        = Mfactor * ( - Qgbo_dVbs + QidLD_dVbs + QisLD_dVbs ) ;
      here->HSMHV2_dQb_dVbsi  +=     here->HSMHV2_cbbo ;
      here->HSMHV2_dQb_dTi    += Mfactor * ( - Qgbo_dT  + QidLD_dT  + QisLD_dT  ) ;

      /* for fringing capacitances */
      here->HSMHV2_dqdp_dVdse += Mfactor * (   Cfd - Qgdo_dVdse ) ;
      here->HSMHV2_dqdp_dVgse += Mfactor * ( - Cfd - Qgdo_dVgse ) ;
      here->HSMHV2_dqdp_dVbse += Mfactor * (       - Qgdo_dVbse ) ;
      here->HSMHV2_dqdp_dTi   += 0.0 ;
      here->HSMHV2_dqsp_dVdse += Mfactor * (       - Qgso_dVdse ) ;
      here->HSMHV2_dqsp_dVgse += Mfactor * ( - Cfs - Qgso_dVgse ) ;
      here->HSMHV2_dqsp_dVbse += Mfactor * (       - Qgso_dVbse ) ;
      here->HSMHV2_dqsp_dTi   += 0.0 ;

      here->HSMHV2_qgext += Mfactor * ( - Qovdext - Qovsext ) ;
      here->HSMHV2_qdext += Mfactor * QbdLDext ;
      here->HSMHV2_qsext += Mfactor * QbsLDext ;
    
      here->HSMHV2_dQdext_dVdse += Mfactor * ( QbdLDext_dVdse ) ;
      here->HSMHV2_dQdext_dVgse += Mfactor * ( QbdLDext_dVgse ) ;
      here->HSMHV2_dQdext_dVbse += Mfactor * ( QbdLDext_dVbse ) ;
      here->HSMHV2_dQdext_dTi   += Mfactor * ( QbdLDext_dT  ) ;
      here->HSMHV2_dQgext_dVdse += Mfactor * ( - Qovdext_dVdse - Qovsext_dVdse  ) ;
      here->HSMHV2_dQgext_dVgse += Mfactor * ( - Qovdext_dVgse - Qovsext_dVgse  ) ;
      here->HSMHV2_dQgext_dVbse += Mfactor * ( - Qovdext_dVbse - Qovsext_dVbse  ) ;
      here->HSMHV2_dQgext_dTi   += Mfactor * ( - Qovdext_dT    - Qovsext_dT ) ;
      here->HSMHV2_dQbext_dVdse += Mfactor * ( QidLDext_dVdse + QisLDext_dVdse ) ;
      here->HSMHV2_dQbext_dVgse += Mfactor * ( QidLDext_dVgse + QisLDext_dVgse ) ;
      here->HSMHV2_dQbext_dVbse += Mfactor * ( QidLDext_dVbse + QisLDext_dVbse ) ;
      here->HSMHV2_dQbext_dTi   += Mfactor * ( QidLDext_dT    + QisLDext_dT ) ;

  }
  here->HSMHV2_dQsi_dVdsi = - (here->HSMHV2_dQdi_dVdsi + here->HSMHV2_dQg_dVdsi + here->HSMHV2_dQb_dVdsi) ;
  here->HSMHV2_dQsi_dVgsi = - (here->HSMHV2_dQdi_dVgsi + here->HSMHV2_dQg_dVgsi + here->HSMHV2_dQb_dVgsi) ;
  here->HSMHV2_dQsi_dVbsi = - (here->HSMHV2_dQdi_dVbsi + here->HSMHV2_dQg_dVbsi + here->HSMHV2_dQb_dVbsi) ;
  here->HSMHV2_dQsi_dTi   = - (here->HSMHV2_dQdi_dTi   + here->HSMHV2_dQg_dTi   + here->HSMHV2_dQb_dTi  ) ;

  here->HSMHV2_dQsext_dVdse = - (here->HSMHV2_dQdext_dVdse + here->HSMHV2_dQgext_dVdse + here->HSMHV2_dQbext_dVdse) ;
  here->HSMHV2_dQsext_dVgse = - (here->HSMHV2_dQdext_dVgse + here->HSMHV2_dQgext_dVgse + here->HSMHV2_dQbext_dVgse) ;
  here->HSMHV2_dQsext_dVbse = - (here->HSMHV2_dQdext_dVbse + here->HSMHV2_dQgext_dVbse + here->HSMHV2_dQbext_dVbse) ;
  here->HSMHV2_dQsext_dTi   = - (here->HSMHV2_dQdext_dTi   + here->HSMHV2_dQgext_dTi   + here->HSMHV2_dQbext_dTi  ) ;

  /*---------------------------------------------------* 
   * Substrate/gate/leak currents.
   *-----------------*/ 

  here->HSMHV2_isub = Mfactor * Isub ;
  here->HSMHV2_dIsub_dVdsi = Mfactor * Isub_dVds ;
  here->HSMHV2_dIsub_dVgsi = Mfactor * Isub_dVgs ;
  here->HSMHV2_dIsub_dVbsi = Mfactor * Isub_dVbs ;
  here->HSMHV2_dIsub_dTi   = Mfactor * Isub_dT  ;
  
  here->HSMHV2_isubld = Mfactor * IsubLD ;
  here->HSMHV2_dIsubLD_dVdsi = Mfactor * IsubLD_dVds ;
  here->HSMHV2_dIsubLD_dVgsi = Mfactor * IsubLD_dVgs ;
  here->HSMHV2_dIsubLD_dVbsi = Mfactor * IsubLD_dVbs ;
  here->HSMHV2_dIsubLD_dTi   = Mfactor * IsubLD_dT  ;
  here->HSMHV2_dIsubLD_dVddp = Mfactor * IsubLD_dVddp ; 
  
  here->HSMHV2_igb   = Mfactor * -Igb ;
  here->HSMHV2_dIgb_dVdsi = - Mfactor * Igb_dVds ;
  here->HSMHV2_dIgb_dVgsi = - Mfactor * Igb_dVgs ;
  here->HSMHV2_dIgb_dVbsi = - Mfactor * Igb_dVbs ;
  here->HSMHV2_dIgb_dTi   = - Mfactor * Igb_dT  ;

  if (here->HSMHV2_mode == HiSIM_NORMAL_MODE) {
    here->HSMHV2_igd   = Mfactor * ( model->HSMHV2_glpart1 * Igate - Igd ) ;
  } else {
    here->HSMHV2_igd   = Mfactor * ( (1.0e0 - model->HSMHV2_glpart1 ) * Igate - Igs ) ;
  }
  
  if (here->HSMHV2_mode == HiSIM_NORMAL_MODE) {
    here->HSMHV2_igs   = Mfactor * ( (1.0e0  - model->HSMHV2_glpart1) * Igate - Igs ) ;
  } else {
    here->HSMHV2_igs   = Mfactor * ( model->HSMHV2_glpart1 * Igate - Igd ) ;
  }
  ...
  here->HSMHV2_dIgd_dVdsi = Mfactor * ( model->HSMHV2_glpart1 * Igate_dVds - Igd_dVds ) ;
  here->HSMHV2_dIgd_dVgsi = Mfactor * ( model->HSMHV2_glpart1 * Igate_dVgs - Igd_dVgs ) ;
  here->HSMHV2_dIgd_dVbsi = Mfactor * ( model->HSMHV2_glpart1 * Igate_dVbs - Igd_dVbs ) ;
  here->HSMHV2_dIgd_dTi   = Mfactor * ( model->HSMHV2_glpart1 * Igate_dT  - Igd_dT  ) ;
  here->HSMHV2_dIgs_dVdsi = Mfactor * ( (1.0 - model->HSMHV2_glpart1) * Igate_dVds - Igs_dVds ) ;
  here->HSMHV2_dIgs_dVgsi = Mfactor * ( (1.0 - model->HSMHV2_glpart1) * Igate_dVgs - Igs_dVgs ) ;
  here->HSMHV2_dIgs_dVbsi = Mfactor * ( (1.0 - model->HSMHV2_glpart1) * Igate_dVbs - Igs_dVbs ) ;
  here->HSMHV2_dIgs_dTi   = Mfactor * ( (1.0 - model->HSMHV2_glpart1) * Igate_dT  - Igs_dT  ) ;
  
  here->HSMHV2_igidl    = Mfactor * Igidl ;
  here->HSMHV2_dIgidl_dVdsi = Mfactor * Igidl_dVds ;
  here->HSMHV2_dIgidl_dVgsi = Mfactor * Igidl_dVgs ;
  here->HSMHV2_dIgidl_dVbsi = Mfactor * Igidl_dVbs ;
  here->HSMHV2_dIgidl_dTi   = Mfactor * Igidl_dT  ;
  
  here->HSMHV2_igisl    = Mfactor * Igisl ;
  here->HSMHV2_dIgisl_dVdsi = Mfactor * Igisl_dVds ;
  here->HSMHV2_dIgisl_dVgsi = Mfactor * Igisl_dVgs ;
  here->HSMHV2_dIgisl_dVbsi = Mfactor * Igisl_dVbs ;
  here->HSMHV2_dIgisl_dTi   = Mfactor * Igisl_dT  ;
  
  /*---------------------------------------------------* 
   * Von, Vdsat.
   *-----------------*/ 
  here->HSMHV2_von = Vth ;
  here->HSMHV2_vdsat = Vdsat ;
   ...
   *-----------------*/
  T1 = here->HSMHV2_ids + here->HSMHV2_dIds_dVdsi + here->HSMHV2_dIds_dVgsi + here->HSMHV2_dIds_dVbsi ;
  T1 = T1 + here->HSMHV2_qd - (here->HSMHV2_dQdi_dVdsi + here->HSMHV2_dQdi_dVgsi + here->HSMHV2_dQdi_dVbsi) ;
  if ( ! isfinite (T1) ) {
    flg_err = 1 ;
    fprintf (stderr ,
    ...
    if ( flg_info >= 1 ) {
      ...
      printf ( "Cds   = %e\n" , -(here->HSMHV2_dQdi_dVdsi
                                                 + here->HSMHV2_dQdi_dVgsi
                                                 + here->HSMHV2_dQdi_dVbsi) ) ;
    }
  }
  
  T1 = here->HSMHV2_isub + here->HSMHV2_dIsub_dVbsi + here->HSMHV2_dIsub_dVdsi + here->HSMHV2_dIsub_dVgsi ;
  if ( ! isfinite (T1) ) {
    flg_err = 1 ;
    fprintf (stderr ,
  ...
  T1 = here->HSMHV2_dQg_dVdsi + here->HSMHV2_dQg_dVgsi + here->HSMHV2_dQg_dVbsi ;
  if ( ! isfinite (T1) ) {
    flg_err = 1 ;
    fprintf(stderr ,
   ...
   * Noise.
   *-----------------*/ 
  here->HSMHV2_noiflick = Mfactor * Nflic ;
  here->HSMHV2_noithrml = Mfactor * Nthrml ;

  /*----------------------------------------------------------*
   * induced gate noise. ( Part 3/3 )
   *----------------------*/
  if ( model->HSMHV2_coign != 0 && model->HSMHV2_cothrml != 0 && flg_ign == 1 && !flg_noqi && model->HSMHV2_codep==0 ) {
    T0 = Cox_small * Cox * here->HSMHV2_weff_nf * Leff ;
    T1 = -( here->HSMHV2_dQg_dVdsi + here->HSMHV2_dQg_dVgsi + here->HSMHV2_dQg_dVbsi ) / Mfactor ; /* NQS case is not supported. */
    if( - T1 > T0 ){
    ...
    correct_w1 = Muun / Mud_hoso ;
      ...
      here->HSMHV2_noiigate = Mfactor * Nign0 * kusai_ig * correct_w1 ;
      here->HSMHV2_noicross = crl_f ;
      if ( here->HSMHV2_noiigate < 0.0 ) here->HSMHV2_noiigate = 0.0e0 ;
    }else{
      here->HSMHV2_noiigate = 0.0e0 ;
      here->HSMHV2_noicross = 0.0e0 ;
    }
  }else{
    here->HSMHV2_noiigate = 0.0e0 ;
    here->HSMHV2_noicross = 0.0e0 ;
  ...
  /* Internal biases */
  if ( here->HSMHV2_called >= 1 ) {
    here->HSMHV2_vbsc_prv2 = here->HSMHV2_vbsc_prv ;
    here->HSMHV2_vdsc_prv2 = here->HSMHV2_vdsc_prv ;
    here->HSMHV2_vgsc_prv2 = here->HSMHV2_vgsc_prv ;
    here->HSMHV2_mode_prv2 = here->HSMHV2_mode_prv ;
  }
  here->HSMHV2_vbsc_prv = Vbs ;
  here->HSMHV2_vdsc_prv = Vds ;
  here->HSMHV2_vgsc_prv = Vgs ;
  here->HSMHV2_mode_prv = here->HSMHV2_mode ;
  here->HSMHV2_temp_prv = TTEMP ;
  
  /* Surface potentials and derivatives w.r.t. internal biases */
  if ( here->HSMHV2_called >= 1 ) {
    here->HSMHV2_ps0_prv2 = here->HSMHV2_ps0_prv ;
    here->HSMHV2_ps0_dvbs_prv2 = here->HSMHV2_ps0_dvbs_prv ;
    here->HSMHV2_ps0_dvds_prv2 = here->HSMHV2_ps0_dvds_prv ;
    here->HSMHV2_ps0_dvgs_prv2 = here->HSMHV2_ps0_dvgs_prv ;
    here->HSMHV2_pds_prv2 = here->HSMHV2_pds_prv ;
    here->HSMHV2_pds_dvbs_prv2 = here->HSMHV2_pds_dvbs_prv ;
    here->HSMHV2_pds_dvds_prv2 = here->HSMHV2_pds_dvds_prv ;
    here->HSMHV2_pds_dvgs_prv2 = here->HSMHV2_pds_dvgs_prv ;
  }

  here->HSMHV2_ps0_prv = Ps0 ;
  here->HSMHV2_ps0_dvbs_prv = Ps0_dVbs ;
  here->HSMHV2_ps0_dvds_prv = Ps0_dVds ;
  here->HSMHV2_ps0_dvgs_prv = Ps0_dVgs ;
  here->HSMHV2_ps0_dtemp_prv = Ps0_dT ;
  here->HSMHV2_pds_prv = Pds ;
  here->HSMHV2_pds_dvbs_prv = Pds_dVbs ;
  here->HSMHV2_pds_dvds_prv = Pds_dVds ;
  here->HSMHV2_pds_dvgs_prv = Pds_dVgs ;
  here->HSMHV2_pds_dtemp_prv = Pds_dT ;


  /* derivatives of channel current w.r.t. external bias (only due to Ra-dependencies!) */
  here->HSMHV2_dIds_dVdse = Ids_dRa * Ra_dVdse * Mfactor ;
  here->HSMHV2_dIds_dVgse = Ids_dRa * Ra_dVgse * Mfactor ;
  here->HSMHV2_dIds_dVbse = Ids_dRa * Ra_dVbse * Mfactor ;
  ...
  if ( VdseModeNML > 0.0 ) {
    ...
    here->HSMHV2_Rs = Rs / Mfactor ;
    here->HSMHV2_dRs_dVdse = Rs_dVdse / Mfactor ;
    here->HSMHV2_dRs_dVgse = Rs_dVgse / Mfactor ;
    here->HSMHV2_dRs_dVbse = Rs_dVbse / Mfactor ;
    here->HSMHV2_dRs_dVsubs = Rs_dVsubs / Mfactor ;
    here->HSMHV2_dRs_dTi   = Rs_dT / Mfactor ;
  } else {
    here->HSMHV2_Rd = Rs / Mfactor ;
    here->HSMHV2_dRd_dVdse = - ( Rs_dVdse + Rs_dVgse + Rs_dVbse + Rs_dVsubs ) / Mfactor ;
    here->HSMHV2_dRd_dVgse = Rs_dVgse / Mfactor ;
    here->HSMHV2_dRd_dVbse = Rs_dVbse / Mfactor ;
    here->HSMHV2_dRd_dVsubs = Rs_dVsubs / Mfactor ;
    here->HSMHV2_dRd_dTi   = Rs_dT / Mfactor ;
    here->HSMHV2_Rs = Rd / Mfactor ;
    here->HSMHV2_dRs_dVdse = - ( Rd_dVdse + Rd_dVgse + Rd_dVbse + Rd_dVsubs ) / Mfactor ;
    here->HSMHV2_dRs_dVgse = Rd_dVgse / Mfactor ;
    here->HSMHV2_dRs_dVbse = Rd_dVbse / Mfactor ;
    here->HSMHV2_dRs_dVsubs = Rd_dVsubs / Mfactor ;
    here->HSMHV2_dRs_dTi   = Rd_dT / Mfactor ;
  }
  /* Clamping to Res_min */
  ...
  if(here->HSMHV2_Rs < Res_min) {
     here->HSMHV2_Rs = Res_min ;
